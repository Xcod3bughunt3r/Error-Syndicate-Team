...
echo -e ${YELLOW}"                         Coded By: ALIF FUSOBAR                               #"
echo -e ${BLUE2}"          {Exposing Other Secret Is Not A Good Idea But U Can Do It}        #"
echo -e ${CP}"          Follow Me On:  ${CPO}Instagram: @xcod3bughunt3r                        #"
echo -e ${CP}"                         ${PINK}Twitter:   @Xcod3bughunt3r                              #"
echo -e ${RED}"#############################################################################\n "
}
if [[ $EUID -ne 0 ]]; then
   echo -e ${RED}"\n[ X ] This script must be run as root :) \n" 1>&2
   echo -e ${RED}"[ ✔ ] Usage: sudo ./pdfcrack.sh"
   exit 1
fi
function depend(){
...
fin=$(hashcat -m 10500 crack.hash -a 3 -i 1?k9?k4?v5?x3?x0 --force --show | awk -F: '{ print $2 }')
if [[ $fin != '' ]]; then
...
hashcat -m 10500  -a 3 crack.hash -i 1?k9?k4?v5?x3?x0 --force >/dev/null 2>&1
...
hashcat -m 10600  -a 3 crack.hash -i 1?k9?k4?v5?x3?x0 --force >/dev/null 2>&1
...
hashcat -m 10700  -a 3 crack.hash -i 1?k9?k4?v5?x3?x0 --force >/dev/null 2>&1
