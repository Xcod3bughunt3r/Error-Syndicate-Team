  ...
  opts = Optimist::options do
    ...
    snmpwn v0.97b
      EOS
        ...
        opt :timeout, "Specify Timeout, for example 0.2 would be 200 milliseconds. Default 0.3", :default => 0.2
...
def findusers(arg, live, cmd)
  ...
  puts "\nEnumerating SNMP Users".light_blue.bold
  live.each do |host|
...
def noauth(arg, users, live, cmd)
  ...
  puts "\nTesting SNMP without authentication and encryption".light_blue.bold
  live.each do |host|
    users.each do |user|
      begin
      out, err = cmd.run!("snmpwalk -u #{user} #{host} iso.3.6.1.2.1.1.1.0")
...
def authnopriv(arg, users, live, passwords, cmd)
  ...
  if !users.empty? and !users.nil?
  puts "\nTesting SNMP with authentication and without encryption".light_blue.bold
  live.each do |host|
    users.each do |user|
...
def authpriv_md5aes(arg, users, live, passwords, cmd, cryptopass)
  ...
  if !users.empty? and !users.nil?
  puts "\nTesting SNMP with MD5 authentication and AES encryption".light_blue.bold
  live.each do |host|
    users.each do |user|
      passwords.each do |password|
        cryptopass.each do |epass|
