...
def menu():
  ...
  if choice == '1' or choice == '01':
    ...
    os.system('clear')
    print (f"{logo}\n{g}INFO SCRIPT\n========================\n{p}[{y}✓{p}] {c}Author: {g}ALIF FUSOBAR\n{p}[{y}✓{p}] {c}Team  : {colors()}MUDA {colors()}MUDI {r}INDO{w}NESIA\n{p}[{y}✓{p}] {c}Script: {colors()}{os.path.basename(sys.argv[0])}\n{p}[{y}✓{p}] {c}Path  : {os.path.realpath(sys.argv[0])}\n{p}[{y}✓{p}] {c}Size  : {os.stat(os.path.realpath(sys.argv[0])).st_size} Byte\n{p}[{y}✓{p}] {c}Link  : {colors()}https://github.com/Xcod3bughunt3r/Virtex\n{p}[{y}✓{p}] {c}Update: {colors()}{UPDATE}\n{p}[{y}✓{p}] {c}Versi : 1.1\n\n{g}Contact Me ^_^\n==================\n{p}[{y}✓{p}] {c}Github: {colors()}https://github.com/Xcod3bughunt3r/\n{p}[{y}✓{p}] {c}Web.   : {colors()}https://www.prisma.id\n{p}[{y}✓{p}] {c}Wa.   : {colors()}+62 82154444485\n{p}[{y}✓{p}] {c}Email : {colors()}admin@prisma.id\n{a}")
